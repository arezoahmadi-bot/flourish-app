...
export default function Timeline() {
  ...
  const getDuration = (task) => {
    ...
    const endMins = (task.end_hour || (task.hour + task.duration) || 0) * 60 + (task.end_minute || 0);
  ...
  const addTimelineTask = async () => {
    ...
    setTimelineTasks([...timelineTasks, ...data].sort((a, b) =>
      (a.start_hour || a.hour) - (b.start_hour || b.hour)));
    setShowForm(false);
    setNewTask({
  ...
  const saveEdit = async (id) => {
    ...
    const { data } = await supabase
      ...
      .eq('id', id)
      .select();
    setTimelineTasks(timelineTasks.map(t => t.id === id ? data[0] : t)
      .sort((a, b) => (a.start_hour || a.hour) - (b.start_hour || b.hour)));
    setEditingTask(null);
    setEditTask(null);
  };
  ...
  const TaskForm = ({ data, setData, onSave, onCancel }) => (
    <div style={{
      borderRadius: '14px',
      padding: '16px',
      ...
      <div style={styles.timeRow}>
        <div style={styles.timeField}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <select
              style={{ ...styles.select, background: theme.card, border: `2px solid ${theme.border}`, color: theme.text }}
              value={data.start_hour}
              onChange={(e) => setData({ ...data, start_hour: parseInt(e.target.value) })}
            >
              {HOURS.map(h => <option key={h} value={h}>{formatHour(h)}</option>)}
            </select>
            <select
              style={{ ...styles.selectSmall, background: theme.card, border: `2px solid ${theme.border}`, color: theme.text }}
              value={data.start_minute}
              onChange={(e) => setData({ ...data, start_minute: parseInt(e.target.value) })}
            >
        ...
        <div style={styles.timeField}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <select
              style={{ ...styles.select, background: theme.card, border: `2px solid ${theme.border}`, color: theme.text }}
              value={data.end_hour}
              onChange={(e) => setData({ ...data, end_hour: parseInt(e.target.value) })}
            >
              {HOURS.map(h => <option key={h} value={h}>{formatHour(h)}</option>)}
            </select>
            <select
              ...
              onChange={(e) => setData({ ...data, end_minute: parseInt(e.target.value) })}
            >
              {minutes.map(m => <option key={m} value={m}>:{String(m).padStart(2, '0')}</option>)}
            </select>
          </div>
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setData({ ...data, category: e.target.value, color: cat?.color || data.color });
          }}
        >
                          ...
                          {editingTask === task.id && editTask ? (
                            ...
                            >
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <p style={{
                                  color: isSleep ? 'rgba(255,255,255,0.7)' : theme.textLight,
                                  fontSize: '0.72rem',
                                }}>
...
const styles = {
  ...
  addBtn: { padding: '10px 20px', borderRadius: '12px', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: '700', fontSize: '0.95rem' },
  card: { borderRadius: '18px', padding: '24px', marginBottom: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.05)' },
  input: { width: '100%', padding: '10px 14px', borderRadius: '10px', fontSize: '0.92rem', outline: 'none', boxSizing: 'border-box' },
  select: { flex: 1, padding: '8px 10px', borderRadius: '10px', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' },
  selectSmall: { width: '70px', padding: '8px 6px', borderRadius: '10px', fontSize: '0.82rem', outline: 'none', boxSizing: 'border-box' },
